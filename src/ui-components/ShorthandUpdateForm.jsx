...
export default function ShorthandUpdateForm(props) {
  const {
    id: idProp,
    shorthand: shorthandModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = shorthandRecord
      ? { ...initialValues, ...shorthandRecord }
      : initialValues;
  ...
  const [shorthandRecord, setShorthandRecord] =
    React.useState(shorthandModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Shorthand, idProp)
        : shorthandModelProp;
      setShorthandRecord(record);
    };
    queryData();
  }, [idProp, shorthandModelProp]);
  React.useEffect(resetStateValues, [shorthandRecord]);
  const validations = {
    User: [{ type: "Required" }],
    key: [{ type: "Required" }],
    value: [{ type: "Required" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Shorthand.copyOf(shorthandRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ShorthandUpdateForm")}
      {...rest}
    >
      <TextField
        label="User"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || shorthandModelProp)}
